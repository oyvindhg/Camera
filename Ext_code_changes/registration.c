...
void freenect_isolate_object(freenect_device* dev, uint16_t* depth_mm, uint8_t* rgb_raw, uint32_t* borders, uint8_t* rgb_registered)
{
	...
	for (y = 0; y < DEPTH_Y_RES; y++) for (x = 0; x < DEPTH_X_RES; x++) {
		...
		if (cx < borders[0] || cy < borders[1] || cx > borders[2] || cy > borders[3]) continue;
